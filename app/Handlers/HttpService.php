...
class HttpService
{
     ...
     * @param bool $return_content 不做处理，直接返回
     * @return bool|string
     */
    public static function request($method, $url, $options = [],$return_content = false)
    {
        $curl = curl_init();
        // GET参数设置
        ...
        if (strtolower($method) === 'post') {
            ...
            // var_dump(json_encode($options['data'],true));
            if ($options['form_data'] ?? []) {
                curl_setopt($curl, CURLOPT_POSTFIELDS, $options['form_data']);
            } else {
                curl_setopt($curl, CURLOPT_POSTFIELDS, json_encode($options['data'], true));
            }
        }

        if (strtolower($method) === 'post-only') {
            curl_setopt($curl, CURLOPT_POST, true);
            // var_dump(json_encode($options['data'],true));
        ...
        if(intval($status["http_code"]) === 200){
            ...
            // log_arr([$options]);
            // log_arr([$content]);
            $msg = sprintf("url:%s options:%s res:%s", $url, json_encode($options), $content);
